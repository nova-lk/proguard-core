...
import proguard.dexfile.reader.api.node.DexCodeNode;
import proguard.dexfile.reader.api.node.DexDebugNode;
import proguard.dexfile.reader.api.node.TryCatchNode;
import proguard.dexfile.reader.api.node.analysis.DvmFrame;
import proguard.dexfile.reader.api.node.analysis.DvmInterpreter;
import proguard.dexfile.reader.api.node.insn.*;
import proguard.dexfile.reader.api.reader.Op;
import proguard.dexfile.reader.api.visitors.DexDebugVisitor;

import java.util.*;

import static proguard.dexfile.ir.expr.Exprs.*;
...
public class Dex2IRConverter {
    ...
    public IrMethod convert(boolean isStatic, Method method, DexCodeNode dexCodeNode) {
        ...
        // Queue all register values (with locals) of all frames.
        ...
        // Iteratively add locals to all parents of all queued values.
        while (!queue.isEmpty()) {
        ...
        if (dexCodeNode.debugNode != null) {
            dexCodeNode.debugNode.accept(new DexDebugVisitor() {
                public void visitLineNumber(int line, DexLabel label) {
                    LabelStmt labelStmt = map.get(label);
                    if (labelStmt != null) {
                        labelStmt.lineNumber = line;
                    }
                }
            });
        }
    ...
    private void addPhi(DvmValue v, Set<Value> phiValues, List<AssignStmt> phis) {
        if (v != null) {
            if (v.local != null) {
                ...
                if (phiValues.size() > 0) {
                    phis.add(nAssign(v.local, nPhi(phiValues.toArray(new Value[phiValues.size()]))));
                    phiValues.clear();
                }
            }
        }
        ...
        while (!stack.isEmpty()) {
            ...
            if (p instanceof DexLabelStmtNode) {
                ...
                if (handlers.get(index)) {
                    Local ex = newLocal();
                    emit(nIdentity(ex, nExceptionRef("Ljava/lang/Throwable;")));
    ...
    private Dex2IrFrame initFirstFrame(DexCodeNode methodNode, IrMethod target) {
        ...
        if (!target.isStatic) {// not static
            Local thiz = newLocal();
            emit(nIdentity(thiz, nThisRef(target.owner)));
            first.setReg(x - 1, new DvmValue(thiz));
        }
        for (int i = 0; i < target.args.length; i++) {
            Local p = newLocal();
            emit(nIdentity(p, nParameterRef(target.args[i], i)));
        ...
        return new DvmInterpreter<DvmValue>() {
            ...
            public DvmValue unaryOperation(DexStmtNode insn, DvmValue value) {
                ...
                switch (insn.op) {
                    ...
                    case IF_EQZ:
                        emit(nIf(nEq(local, nInt(0), TypeClass.ZIL.name), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_GEZ:
                        emit(nIf(nGe(local, nInt(0), "I"), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_GTZ:
                        emit(nIf(nGt(local, nInt(0), "I"), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_LEZ:
                        emit(nIf(nLe(local, nInt(0), "I"), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_LTZ:
                        emit(nIf(nLt(local, nInt(0), "I"), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_NEZ:
                        emit(nIf(nNe(local, nInt(0), TypeClass.ZIL.name), getLabel(((JumpStmtNode) insn).label)));
                        return null;
            ...
            public DvmValue binaryOperation(DexStmtNode insn, DvmValue value1, DvmValue value2) {
                ...
                switch (insn.op) {
                    ...
                    case IF_EQ:
                        emit(nIf(nEq(local1, local2, TypeClass.ZIL.name), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_GE:
                        emit(nIf(nGe(local1, local2, "I"), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_GT:
                        emit(nIf(nGt(local1, local2, "I"), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_LE:
                        emit(nIf(nLe(local1, local2, "I"), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_LT:
                        emit(nIf(nLt(local1, local2, "I"), getLabel(((JumpStmtNode) insn).label)));
                        return null;

                    case IF_NE:
                        emit(nIf(nNe(local1, local2, TypeClass.ZIL.name), getLabel(((JumpStmtNode) insn).label)));
                        return null;
