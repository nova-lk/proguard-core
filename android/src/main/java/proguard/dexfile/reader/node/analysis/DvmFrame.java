...
import proguard.dexfile.reader.node.insn.*;
    ...
    public void execute(DexStmtNode insn, DvmInterpreter<V> interpreter) {
        ...
        switch (insn.op) {
            ...
            case INVOKE_POLYMORPHIC_RANGE: {
                ...
                int startReg = isRange
                        ? methodStmtNode.args[0] // starting register is always included in args for invoke-*/range
                        : 0;                     // unused dummy value, args.length may be 0 (e.g. plain invoke-static with no arguments)

                for (String type : proto.getParameterTypes()) {
                    v.add(getReg(isRange ? startReg + i : methodStmtNode.args[i]));
