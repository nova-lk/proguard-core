...
public class TypeTransformer implements Transformer {
    ...
    private static class TypeAnalyze {
                ...
                for (TypeRef ref : arrayRoots) {
                    ...
                    if (provideDesc != null && provideDesc.charAt(0) == '[') {
                        ...
                        if (ref.gArrayValues != null) {
                            ...
                            // Array values are linked so if there is a type update, we need
                            // to fix all linked array values as well.
                            updateLinkedValues(updatedRefs, ref.gArrayValues, q);
                        }
                        if (ref.sArrayValues != null) {
                            ...
                            for (TypeRef p : ref.sArrayValues) {
                                ...
                                // Array values are linked so if there is a type update, we need
                                // to fix all linked array values as well.
                                updateLinkedValues(updatedRefs, ref.sArrayValues, q);
                            }
                        }
                    }
                }
        ...
        private static void updateLinkedValues(Set<TypeRef> updatedRefs, Set<TypeRef> referencedValues, UniqueQueue<TypeRef> q) {
            for (TypeRef p : referencedValues) {
                TypeRef real = p.getReal();
                if (p != real && updatedRefs.contains(real)) {
                    q.add(p);
                }
            }
        }
        ...
        private void linkSameAs(Value a, Value b) {
            ...
            if (aa.sameValues == null) {
                aa.sameValues = new LinkedHashSet<>(3);
            }
            if (bb.sameValues == null) {
                bb.sameValues = new LinkedHashSet<>(3);
            }
        ...
        private void linkGetArray(Value array, Value v) {
            ...
            if (root.gArrayValues == null) {
                root.gArrayValues = new LinkedHashSet<>(3);
            }
            root.gArrayValues.add(value);
            if (value.arrayRoots == null) {
                value.arrayRoots = new LinkedHashSet<>(3);
            }
        ...
        private void linkSetArray(Value array, Value v) {
            ...
            if (root.sArrayValues == null) {
                root.sArrayValues = new LinkedHashSet<>(3);
            }
        ...
        private void linkFromTo(Value from, Value to) {
            ...
            if (tFrom.children == null) {
                tFrom.children = new LinkedHashSet<>();
            }
            tFrom.children.add(tTo);
            if (tTo.parents == null) {
                tTo.parents = new LinkedHashSet<>();
            }
