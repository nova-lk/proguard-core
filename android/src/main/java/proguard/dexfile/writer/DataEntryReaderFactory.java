...
public class DataEntryReaderFactory
{
     ...
     * @param reader         a data entry reader to which the reading of actual
     *                       classes and resource files can be delegated.
     * @param out            a print stream, to which is printed the messages
     * @return a DataEntryReader for reading the given class path entry.
     */
    public DataEntryReader createDataEntryReader(String          messagePrefix,
                                                 ClassPathEntry classPathEntry,
            ...
            if (android)
            {
                jarMatcher =
                    new AndMatcher(
                        new AndMatcher(
                            new NotMatcher(
                            new ListParser(new FileNameParser()).parse(AndroidConstants.ASSETS_FILE_EXPRESSION)),

                            new NotMatcher(
                            new ListParser(new FileNameParser()).parse(AndroidConstants.RES_FILE_EXPRESSION))),
